...
async function run() {
  try {
    ...
    const registry_auth_tlscontext = getStringInput('registry_auth_tlscontext');
    // ----- action ux -----
    ...
    const args = [`${debug}`, `${addr}`, `${log_format}`, `${tlsdir}`, 'build', `${output}`, `${progress}`, `${local}`, `${frontend}`, `${opt}`, `${no_cache}`, `${export_cache}`, `${import_cache}`, `${secret}`, `${allow}`, `${ssh}`, `${registry_auth_tlscontext}`].filter(arg => arg !== '');
